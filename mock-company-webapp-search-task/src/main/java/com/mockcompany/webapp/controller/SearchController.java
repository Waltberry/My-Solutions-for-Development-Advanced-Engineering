...
public class SearchController {
    ...
    public Collection<ProductItem> search(@RequestParam("query") String query) {
        ...
        for (ProductItem item : allItems) {
            ...
            boolean matchesSearch = isQuoted ? itemMatchesQuotedQuery(item, query) : itemMatchesQuery(item, query);
            if (matchesSearch) {
                itemList.add(item);
            }
            // itemList.add(item);
...
// Helper method to check if the item matches the query (without quotes)
private boolean itemMatchesQuery(ProductItem item, String query) {
    return item.getName().toLowerCase().contains(query.toLowerCase()) ||
           item.getDescription().toLowerCase().contains(query.toLowerCase());
}

// Helper method to check if the item matches the quoted query
private boolean itemMatchesQuotedQuery(ProductItem item, String query) {
    return item.getName().equalsIgnoreCase(query) || item.getDescription().equalsIgnoreCase(query);
}
}